(function(){
    ...
    function initializeMenu(){
        currentModal = document.getElementById('modal-main');
        currentModalName = 'main';
        var menuItems = document.querySelectorAll('li.menu-item');
        for(var i = 0; i < menuItems.length; i++){
            var menuItem = menuItems[i];
            menuItem.onclick = menuItemOnClick.bind(menuItem);
        }
    }
    ...
    function openModal(target){
        ...
        currentModal = document.getElementById('modal-' + target)
        currentModalName = target;
    }

    function closeModal(target){
        shrink(target);
        grow(document.getElementById('modal-main'));
    }

    function grow(target, callback){
        fadeIn(target, 250, callback);
    }

    function shrink(target, callback){
        fadeOut(target, 250, callback);
    }
    ...
    function fadeOut(element, duration, callback) {
        ...
        var startTime = Date.now();
        var tick = function() {
            var newOpacity = 1 - (Date.now() - startTime) / duration;
            if(newOpacity < 0){
                newOpacity = 0;
